  ...
  const Form = () => {
    return type === "delete" && id ? (
      <form action="" className="flex flex-col gap-4 p-4">
        ...
        <button type='button' className="w-max self-center py-2 px-4 bg-red-700 text-white rounded-md border-none">
          Delete
        </button>
      </form>
        ...
        {open && (
          <div className="absolute left-0 top-0 flex items-center justify-center w-screen h-screen z-50 bg-black bg-opacity-60">
            <div className="relative w-[90%] md:w-[70%] lg:w-[60%] xl:w-[50%] 2xl:w-[40%] p-4 rounded-md bg-white">
              <button type='button' className='absolute top-4 right inline-flex items-center justify-center-4 cursor-pointer' onClick={() => setOpen(false)}>
